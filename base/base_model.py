...
class BaseModel:
    ...
    def init_global_step(self):
        # don't forget to add the global step to the tensorflow trainer
        with tf.variable_scope('global_step'):
            self.global_step_tensor = tf.Variable(0, trainable=False, name='global_step')
            # this operator if you wanna increment the global_step_tensor by yourself instead of
            # incrementing it by .minimize function in the optimizers of tensorflow
            self.increment_global_step_tensor = tf.assign_add(self.global_step_tensor, 1)
